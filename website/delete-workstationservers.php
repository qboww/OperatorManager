            ...
            <div class="col-md-6">
                <div class="card">
                    <div class="card-header">
                        Delete Workstation-Server relation
                    </div>
                    <div class="card-body">
                        <?php
                        if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["delete"])) {
                            ...
                            if (!empty($workstationID) && !empty($serverID)) {
                                if ($conn) {
                                    $checkQuery = "SELECT * FROM workstationservers WHERE WorkstationID = $workstationID AND ServerID = $serverID";
                                    $result = $conn->query($checkQuery);

                                    if ($result->num_rows > 0) {
                                        $deleteQuery = "DELETE FROM workstationservers WHERE WorkstationID = $workstationID AND ServerID = $serverID";
                                        ...
                                        echo '<div class="alert alert-danger" role="alert">Record with given Workstation ID and Server ID does not exist.</div>';
                                    }
                                }
                            } else {
                                echo '<div class="alert alert-danger" role="alert">Both Workstation ID and Server ID are required.</div>';
                            }
                        }
                        ?>
                        <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                            <div class="form-group">
                                ...
                                <label class="mt-2">Server ID</label>
                                <input type="text" class="form-control" id="serverID" name="serverID" placeholder="Enter server id">
