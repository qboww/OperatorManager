            ...
            <div class="col-md-6">
                <div class="card">
                    <div class="card-header">
                        Delete Operator-Workstation relation
                    </div>
                    <div class="card-body">
                        <?php
                        if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["delete"])) {
                            ...
                            if (!empty($operatorID) && !empty($workstationID)) {
                                if ($conn) {
                                    $checkQuery = "SELECT * FROM operatorworkstations WHERE OperatorID = $operatorID AND WorkstationID = $workstationID";
                                    $result = $conn->query($checkQuery);

                                    if ($result->num_rows > 0) {
                                        $deleteQuery = "DELETE FROM operatorworkstations WHERE OperatorID = $operatorID AND WorkstationID = $workstationID";
                                        ...
                                        echo '<div class="alert alert-danger" role="alert">Record with given Operator ID and Workstation ID does not exist.</div>';
                                    }
                                }
                            } else {
                                echo '<div class="alert alert-danger" role="alert">Both Operator ID and Workstation ID are required.</div>';
                            }
                        }
                        ?>
                        <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="operatorID" name="operatorID" placeholder="Enter operator id">

                                <label class="mt-2">Workstation ID</label>
